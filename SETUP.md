...
6. `0006_payment_status.sql` — adds `payment_status` to orders
7. `0007_custom_requests_and_tracking.sql` — the **Custom Request** ("we'll
   source it") table + its public image bucket, and the `track_order()`
   function that powers **Track My Order**. Run this even on an existing,
   already-live project — it only adds new things, it doesn't touch existing
   data.
