...
import { Clock, MapPin, Phone, Sparkles, Truck, ShieldCheck, PackageSearch } from 'lucide-react';
...
const promises: {icon: typeof Sparkles;title: string;text: string;to?: string;}[] = [
{
  icon: Sparkles,
...
{
  icon: PackageSearch,
  title: "Can't find it? We source it",
  text: 'Send us a photo or describe it — we find it and price it for you.',
  to: '/custom-request'
}];
    ...
    <div className="w-full bg-ivory">
      ...
      <section className="border-b border-cocoa/10 bg-cream" aria-label="Why shop with us">
        <ul className="container grid gap-6 py-8 sm:grid-cols-2 lg:grid-cols-4">
          {promises.map(({ icon: Icon, title, text, to }) => {
            const content = (
              <>
            ...
            return (
              <li key={title} className="flex gap-3">
                {to ?
                <Link to={to} className="flex gap-3 hover:text-gold">
                    {content}
                  </Link> :
