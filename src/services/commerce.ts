...
: Promise<{ok: boolean;message: string;order?: Order;}> {
  if (!reference.trim() || !contact.trim()) {
    return { ok: false, message: 'Enter your order number and the email or phone you used.' };
  }
  const { data, error } = await supabase.rpc('track_order', {
    p_reference: reference.trim(),
    p_contact: contact.trim()
  });
  if (error) return { ok: false, message: 'Could not look that up right now — please try again.' };
  const row = Array.isArray(data) ? data[0] : data;
  if (!row) {
    return {
      ok: false,
      message: "We couldn't find an order with that number and contact detail. Double-check both and try again."
    };
  }
  return { ok: true, message: 'Order found.', order: toOrder(row) };
}
